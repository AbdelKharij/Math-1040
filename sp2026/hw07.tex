...
multiplication principle, permutations, distinguishable permutations, or combinations --- and then carrying out the
count.
\end{abstract}
\maketitle
...
\begin{problem}
For each of the following situations, use the multiplication principle, permutations,
\emph{distinguishable} permutations, or combinations to count appropriately.
...
\begin{prompt}
\textbf{(e)} If you have $4$ red tiles, $5$ yellow tiles, and $3$ blue tiles to place in
a row, how many distinguishable patterns are possible?
...
The count is $\answer{27720}$.

\begin{hint}
Arrange all $12$ tiles as if they were different, then divide out the rearrangements of
same-colored tiles, which don't change the pattern.
\end{hint}

\begin{feedback}
\[ \frac{12!}{4!\,5!\,3!}=27{,}720. \]
\end{feedback}
\end{prompt}
\end{problem}
