...
\begin{document}
\begin{abstract}
Exam 1, covering Sections 7.1--7.3: set identities and Venn diagrams,
...
\begin{hint}
Look at the part of $C$ that lies inside $B$. Is it shaded? Should it be?
\end{hint}

\begin{feedback}
False. The diagram shades the part of $A$ outside $B$, together with \emph{all} of $C$
--- including the part of $C$ inside $B$. So it shows
$\left(A\cap\olsi{B}\right)\cup C$.

The claimed set $\left(A\cup C\right)\cap\olsi{B}$ requires everything shaded to lie
outside $B$, so the portion of $C$ inside $B$ would have to be left blank.
\end{feedback}
\end{prompt}
\end{problem}
