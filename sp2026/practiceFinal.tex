...
expected value as a decision tool, and a mixed-strategy equilibrium.
\end{abstract}
\maketitle
...
\begin{prompt}
\textbf{(d)} The following payoff matrix represents Alice's perspective for a zero-sum
game. Determine Bob's stable mixed strategy at the \emph{equilibrium point} of this
game. Enter fractions.
\[
\begin{array}{c|c|c|}
\phantom{|} & \text{Bob: }a & \text{Bob: }b \\\hline
\text{Alice: }a & 2 & 1 \\\hline
\text{Alice: }b & 0 & 3 \\\hline
\end{array}
\]

$P(\text{Bob plays } a)=\answer{1/2}$ \qquad $P(\text{Bob plays } b)=\answer{1/2}$

\begin{hint}
First check that there is no saddle point. Then let Bob play $a$ with probability $q$,
and choose $q$ so that Alice's two strategies have the same expected value --- that way
she has nothing to exploit.
\end{hint}

\begin{feedback}
There is no saddle point: the row minima $1$ and $0$ give a maximin of $1$, and the
column maxima $2$ and $3$ give a minimax of $2$.

Let Bob play $a$ with probability $q$ and $b$ with probability $1-q$. Alice's expected
payoff is
\[
\text{row } a:\ 2q+1(1-q)=1+q, \qquad \text{row } b:\ 0q+3(1-q)=3-3q.
\]
At equilibrium these are equal: $1+q=3-3q$, so $q=\frac12$. Bob plays each strategy
half the time, and the value of the game is $1+\frac12=\frac32$ to Alice.

(For completeness, Alice's equilibrium mix is found the same way: playing $a$ with
probability $p$ gives $2p$ against Bob's $a$ and $3-2p$ against Bob's $b$, equal when
$p=\frac34$.)
\end{feedback}
\end{prompt}
\end{problem}
